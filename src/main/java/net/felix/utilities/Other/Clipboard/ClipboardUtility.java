...
public class ClipboardUtility {
    ...
    // KeyMapping für das Togglen des Clipboards
    private static KeyMapping toggleClipboardKeyMapping;
    private static KeyMapping nextPageKeyMapping;
    private static KeyMapping previousPageKeyMapping;
    ...
    public static void initialize() {
        // WICHTIG: KeyMapping ZUERST registrieren (wie bei KillsUtility und BPViewerUtility)
        ...
        HudElementRegistry.addLast(Identifier.fromNamespaceAndPath("cclive-utilities", "clipboard"), (drawContext, tickDelta) -> {
            net.minecraft.client.Minecraft client = net.minecraft.client.Minecraft.getInstance();
            if (client == null) return;
            
            // Nur rendern wenn kein Screen offen ist (außerhalb von Inventaren)
            if (client.screen == null) {
                int mouseX = (int) client.mouseHandler.xpos();
                int mouseY = (int) client.mouseHandler.ypos();
                float delta = 1.0f; // Delta wird nicht benötigt für statisches Rendering
                net.felix.utilities.DragOverlay.Overall.ClipboardDraggableOverlay.renderInGame(drawContext, mouseX, mouseY, delta);
                
                // Render Button Tooltips
                net.felix.utilities.DragOverlay.Overall.ClipboardDraggableOverlay.renderButtonTooltips(drawContext, mouseX, mouseY);
            }
        });
        ...
        net.fabricmc.fabric.api.client.event.lifecycle.v1.ClientTickEvents.END_CLIENT_TICK.register(client -> {
            if (client.screen == null && client.mouseHandler != null) {
                // Prüfe ob Maus geklickt wurde (wird in einem Mixin behandelt)
                // Für jetzt: Button-Clicks werden in AbstractContainerScreenMixin und ScreenMixin behandelt
            }
            ...
            net.felix.utilities.DragOverlay.Overall.ClipboardDraggableOverlay.updateHideHover();
            ...
            if (toggleClipboardKeyMapping != null && toggleClipboardKeyMapping.consumeClick()) {
    ...
    private static void handlePageHotkeys(net.minecraft.client.Minecraft client) {
        if (!canHandleClipboardPageHotkeys(client)) {
            return;
        }
        if (previousPageKeyMapping != null && previousPageKeyMapping.consumeClick()) {
            goToPreviousPage();
        }
        if (nextPageKeyMapping != null && nextPageKeyMapping.consumeClick()) {
            goToNextPage();
        }
    }

    private static boolean canHandleClipboardPageHotkeys(net.minecraft.client.Minecraft client) {
    ...
    /**
     * Registriert KeyMappings für das Clipboard
     */
    private static void registerKeyMapping() {
        toggleClipboardKeyMapping = KeyMappingHelper.registerKeyMapping(new KeyMapping(
        ...
        nextPageKeyMapping = KeyMappingHelper.registerKeyMapping(new KeyMapping(
            "key.cclive-utilities.clipboard.next-page",
            InputConstants.Type.KEYSYM,
            GLFW.GLFW_KEY_UNKNOWN,
            KeyCategories.of("cclive-utilities", "itemviewer")
        ));
        previousPageKeyMapping = KeyMappingHelper.registerKeyMapping(new KeyMapping(
    ...
    public static boolean handleKeyPress(int keyCode, int scanCode) {
        try {
            ...
            if (canHandleClipboardPageHotkeys(client)) {
                if (nextPageKeyMapping != null && nextPageKeyMapping.matches(new net.minecraft.client.input.KeyEvent(keyCode, scanCode, 0))) {
                    goToNextPage();
                    return true;
                }
                if (previousPageKeyMapping != null && previousPageKeyMapping.matches(new net.minecraft.client.input.KeyEvent(keyCode, scanCode, 0))) {
                    goToPreviousPage();
                    return true;
                }
            }

            // Prüfe ob der gedrückte Key dem konfigurierten KeyMapping entspricht
            if (toggleClipboardKeyMapping != null) {
                // Verwende matchesKey um zu prüfen, ob der gedrückte Key dem konfigurierten KeyMapping entspricht
                if (toggleClipboardKeyMapping.matches(new net.minecraft.client.input.KeyEvent(keyCode, scanCode, 0))) {
     ...
     */
    private static boolean isTextFieldFocused(net.minecraft.client.Minecraft client) {
